...
def normalize_speed_percent(value):
...
class AgxArmRosNode(Node):
    ...
    def __init__(self):
        ...
        ### publisher thread
        self._last_invalid_joint_feedback_warning_time = 0.0
        self._last_feedback_unavailable_warning_time = 0.0
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter("preserve_controller_state", False)
    ...
    def _load_parameters(self):
        ...
        self.preserve_controller_state = self.get_parameter("preserve_controller_state").value
        ...
        self._pending_speed_percent = None
    ...
    def _log_parameters(self):
        ...
        self.get_logger().info(f"preserve_controller_state: {self.preserve_controller_state}")
    ...
    def _on_set_parameters(self, parameters):
        speed_parameters = [
            parameter for parameter in parameters if parameter.name == "speed_percent"
        ]
        if not speed_parameters:
            return SetParametersResult(successful=True)

        try:
            speed_percent = normalize_speed_percent(speed_parameters[-1].value)
            if self.preserve_controller_state:
                self._pending_speed_percent = speed_percent
            else:
                self.agx_arm.set_speed_percent(speed_percent)
        except Exception as exception:
            return SetParametersResult(successful=False, reason=str(exception))

        self.speed_percent = speed_percent
        suffix = " (pending next arm motion)" if self.preserve_controller_state else ""
        self.get_logger().info(f"speed_percent updated to {speed_percent}{suffix}")
        return SetParametersResult(successful=True)

    def _init_agx_arm(self):
        if self.preserve_controller_state:
            if self.auto_enable:
                raise ValueError("preserve_controller_state requires auto_enable=false")
            if not math.isfinite(self.enable_timeout) or self.enable_timeout <= 0:
                raise ValueError("enable_timeout must be finite and positive")
            self._pending_speed_percent = normalize_speed_percent(self.speed_percent)
        try:
            self._connect_agx_arm()
        except BaseException:
            if self.preserve_controller_state and hasattr(self, "agx_arm"):
                self.enable_flag = False
                self.control_ready = False
                self.agx_arm.disconnect()
            raise

    def _connect_agx_arm(self):
        ...
        elif not self.preserve_controller_state:
            time.sleep(0.1)
        ...
        if self.preserve_controller_state:
            self._verify_preserved_enable(connected_at)
            self.get_logger().info(
                "Controller state preserved; speed_percent pending next arm motion"
            )
        else:
            self.agx_arm.set_speed_percent(self.speed_percent)
        self.agx_arm.set_tcp_offset(self.tcp_offset)

    def _verify_preserved_enable(self, connected_at):
        deadline = time.monotonic() + self.enable_timeout
        while time.monotonic() < deadline:
            states = [self.agx_arm.get_driver_states(i)
                      for i in range(1, self.arm_joint_count + 1)]
            now = time.time()
            # SDK timestamps are SocketCAN wall-clock receive timestamps. Never
            # accept cached pre-connection frames or mix old enabled states.
            if states and all(
                state is not None
                and math.isfinite(state.timestamp)
                and connected_at <= state.timestamp <= now
                and now - state.timestamp <= 0.5
                and math.isfinite(state.hz) and state.hz > 0
                and state.msg.foc_status.driver_enable_status
                for state in states
            ):
                self.enable_flag = True
                return
            time.sleep(0.01)
        raise RuntimeError("Timed out verifying fresh enabled feedback for every joint")
    ...
    def _publish_thread(self):
        ...
        while rclpy.ok():
            if self.agx_arm.is_ok():
                ...
                now = time.monotonic()
                if now - self._last_feedback_unavailable_warning_time >= 1.0:
                    self.get_logger().warning("AGX arm feedback is unavailable")
                    self._last_feedback_unavailable_warning_time = now
            rate.sleep()
    ...
    def _publish_joint_states(self):
        ...
        for joint_index in range(1, self.arm_joint_count+1):
            ...
            if ms is None:
                now = time.monotonic()
                if now - self._last_invalid_joint_feedback_warning_time >= 1.0:
                    self.get_logger().warning(
                        f"Skipping joint feedback frame: motor {joint_index} state unavailable"
                    )
                    self._last_invalid_joint_feedback_warning_time = now
                return
        ...
        if joints_data:
            names, positions, velocities, efforts = map(list, zip(*joints_data))
            numeric_fields = {
                "position": finite_float_list(positions),
                "velocity": finite_float_list(velocities),
                "effort": finite_float_list(efforts),
            }
            invalid_fields = [
                name for name, values in numeric_fields.items() if values is None
            ]
            if invalid_fields:
                now = time.monotonic()
                if now - self._last_invalid_joint_feedback_warning_time >= 1.0:
                    self.get_logger().warning(
                        "Skipping invalid joint feedback frame: "
                        + ", ".join(invalid_fields)
                    )
                    self._last_invalid_joint_feedback_warning_time = now
                return
            msg.name = names
            msg.position = numeric_fields["position"]
            msg.velocity = numeric_fields["velocity"]
            msg.effort = numeric_fields["effort"]
    ...
    def _publish_arm_status(self):
        ...
        msg.err_status = arm_status.msg.err_code
    ...
    def _valid_handoff_joint_command(self, msg, allow_effector_only=False):
        if not self.preserve_controller_state:
            return True
        if allow_effector_only and not any(name in self.arm_joint_names for name in msg.name):
            return True
        if (len(msg.name) == len(msg.position)
                and len(set(msg.name)) == len(msg.name)
                and all(name in msg.name for name in self.arm_joint_names)
                and finite_float_list(msg.position) is not None):
            return True
        self.get_logger().error("Ignoring incomplete or invalid arm joint command")
        return False

    def _dispatch_arm_motion(self, motion, *targets, **kwargs):
        if self.preserve_controller_state:
            if motion != "move_mit":
                expected = self.arm_joint_count if motion in ("move_j", "move_js") else 6
                if any(len(target) != expected or finite_float_list(target) is None
                       for target in targets):
                    raise ValueError("Invalid arm motion target")
            if self._pending_speed_percent is not None:
                self.agx_arm.set_speed_percent(self._pending_speed_percent)
                self._pending_speed_percent = None
        getattr(self.agx_arm, motion)(*targets, **kwargs)
    ...
    def _control_arm_joints(self, joint_pos):
        ...
        if arm_joints:
            ...
            if self.fast_mode:
                self._dispatch_arm_motion("move_js", joints)
                self.is_mit_mode = True
            else:
                self._dispatch_arm_motion("move_j", joints)
                self.is_mit_mode = False
    ...
    def _joint_states_callback(self, msg: JointState):
        if not self._check_can_control():
            return
        if not self._valid_handoff_joint_command(msg, allow_effector_only=True):
            return
    ...
    def _move_j_callback(self, msg: JointState):
        if not self._check_can_control():
            return
        if not self._valid_handoff_joint_command(msg):
            return
    ...
    def _move_p_callback(self, msg: PoseStamped):
        ...
        self._dispatch_arm_motion("move_p", pose_cmd)
        self.is_mit_mode = False
    ...
    def _move_l_callback(self, msg: PoseStamped):
        ...
        pose_cmd = self._create_pose_cmd(msg.pose)
        self._dispatch_arm_motion("move_l", pose_cmd)
        self.is_mit_mode = False
    ...
    def _move_c_callback(self, msg: PoseArray):
        ...
        self._dispatch_arm_motion("move_c", pose_start, pose_mid, pose_end)
        self.is_mit_mode = False
    ...
    def _move_mit_callback(self, msg: MoveMITMsg):
        ...
        if not arrays[0]:
            ...
            return
        if self.preserve_controller_state and (
            any(index not in range(1, self.arm_joint_count + 1) for index in msg.joint_index)
            or any(finite_float_list(values) is None for values in arrays[1:])
        ):
            self.get_logger().error("Ignoring invalid MIT arm command")
            return
        ...
        for i in range(len(msg.joint_index)):
            ...
            self._dispatch_arm_motion("move_mit", **params)
        self.is_mit_mode = True
    ...
    def _move_home_callback(self, request, response):
        try:
            if not self._check_arm_ready():
                ...
                if self.is_mit_mode:
                    self._dispatch_arm_motion("move_js", [0] * self.arm_joint_count)
                else:
                    self._dispatch_arm_motion("move_j", [0] * self.arm_joint_count)
    ...
    def _emergency_stop_callback(self, request, response):
        ...
        try:
            ...
            q = list(js.msg)
            if not self.is_switch_seamlessly:
                self._dispatch_arm_motion("move_js", q)
                self.is_mit_mode = True
            else:
                self._dispatch_arm_motion("move_j", q)
                self.is_mit_mode = False
